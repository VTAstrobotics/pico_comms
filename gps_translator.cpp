...
void handle_navsat_publishing(rcl_publisher_t *publisher, sensor_msgs__msg__NavSatFix *msg)
{
    ...
    if (ready_to_publish)
    {
        ...
        std::string gps1 = cpp_string(gps_buffer_internal);
        char *gps_data = strtok(gps1, delimiter); //separate GPS into fields
        gps_fields.push_back(std::string(gps_fields)); // add GPS to vector then extract long and lat
